...
The implemented routes are `/`, `/menu`, `/story`, `/gallery`, `/private-dining`, `/visit`, `/faq`, and `/privacy`. The private-dining form prepares an email in the visitor's mail app because no server-side submission provider is configured; its copy states this behavior. Default site styling deliberately does not switch with OS light/dark preference.
